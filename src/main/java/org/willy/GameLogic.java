...
class GameLogic {
    ...
    /**
     * Recovers which direction the combo piece last travelled in. Callers only
     * ever carry {@code prevPos} and {@code comboPiece} across the wire, so the
     * direction has to be re-derived from the adjacency table — the capture
     * chain rules need it to forbid continuing in the same direction.
     *
     * @return the direction index, or -1 if the two squares aren't adjacent
     */
    static int deriveLastDir(int prevPos, int comboPiece) {
        for (int d = 0; d < 8; d++) if (ADJ_INDEX[prevPos][d] == comboPiece) return d;
        return -1;
    }
